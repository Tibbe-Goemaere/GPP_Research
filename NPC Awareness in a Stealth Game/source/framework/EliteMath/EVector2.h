...
#pragma region ExtraFunctions
	...
	/*! Area of triangle !*/
	inline float Area(const Elite::Vector2& edge0, const Elite::Vector2& edge1, const Elite::Vector2& edge2)
	{
		const float area{ abs(edge0.x * (edge1.y - edge2.y) + edge1.x * (edge2.y - edge0.y) + edge2.x * (edge0.y - edge1.y)) / 2.f };
		return area;
	}
